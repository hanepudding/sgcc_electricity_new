...
def main():
    ...
    if os.path.isfile('/data/options.json'):
        ...
        try:
            ...
            logger.info(f"当前以Homeassistant Add-on 形式运行.")
            import const
            ...
            logger.error(f"读取 options.json 文件失败，程序将退出，错误信息: {e}。")
            sys.exit()

    try:
        PHONE_NUMBER = os.getenv("PHONE_NUMBER")
        logger.info(f"读取环境变量 PHONE_NUMBER : {PHONE_NUMBER}")
        ...
        logger.error(f"读取 .env 文件失败，程序将退出，错误信息: {e}。")
        sys.exit()

    logger.info(f"当前仓库版本为 {VERSION}，仓库地址为 https://github.com/ARC-MX/sgcc_electricity_new.git")
    current_datetime = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    logger.info(f"当前日期为 {current_datetime}。")

    logger.info(f"开始初始化 ErrorWatcher")
    ErrorWatcher.init(root_dir='/data/errors')
    logger.info(f'ErrorWatcher 初始化完成！')
    ...
    logger.info(f"当前登录用户名为 {PHONE_NUMBER}，Home Assistant 地址为 {HASS_URL}，程序将每天在 {parsed_time.strftime('%H:%M')} 执行。")

    # 添加随机延迟
    next_run_time = parsed_time + timedelta(hours=12)

    logger.info(f'立即执行任务！下次运行时间为每天 {parsed_time.strftime("%H:%M")} 和 {next_run_time.strftime("%H:%M")}')
    ...
    if not updator.republish():
        logger.info("未找到有效缓存，正在从国家电网获取数据...")
        run_task(fetcher)
    else:
        logger.info("已从缓存恢复数据，跳过启动时抓取以保护账号。")
...
def run_task(data_fetcher: DataFetcher):
    for retry_times in range(1, RETRY_TIMES_LIMIT + 1):
        try:
            ...
            logger.error(f"状态刷新任务失败，原因是 [{e}]，还剩 {RETRY_TIMES_LIMIT - retry_times} 次重试机会。")
            continue
