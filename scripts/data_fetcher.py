...
class DataFetcher:
    ...
    def _get_webdriver(self):
        ...
        # CloakBrowser 隐身参数（指纹随机化）
        for arg in cloakbrowser.get_default_stealth_args():
            chrome_options.add_argument(arg)

        # 使用 CloakBrowser 的修补版 Chromium 二进制文件 + 配套 ChromeDriver
        binary_path = cloakbrowser.ensure_binary()
        chrome_options.binary_location = binary_path
        cache_dir = cloakbrowser.binary_info()['cache_dir']
        chromedriver_path = os.path.join(cache_dir, 'chromedriver')
